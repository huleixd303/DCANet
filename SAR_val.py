...
def SAR_val(args):
    ...
    norm = transforms.Normalize([0.485, 0.456, 0.406],
                             [0.229, 0.224, 0.225])
    # norm =transforms.Normalize([0.406, 0.428, 0.394],
    #                             [0.201, 0.183, 0.176])
    ...
    model.to(device)
    print("Read Input Image from : {}".format(args.img_path))
    imgset = glob.glob(os.path.join(args.img_path, '*.jpg'))
    print(len(imgset))
    imgname = os.listdir(args.img_path)

    print(len(imgname))
    for ids in imgname:
        out_ids_name = os.path.split(ids)[1]
        gt_name = out_ids_name.split('.')[0] + ".png"

        gt_path = os.path.join(args.gt_path, gt_name)
        gt = Image.open(gt_path)
        gt = np.array(gt)

        ids_path = os.path.join(args.img_path, ids)
        img = Image.open(ids_path)
