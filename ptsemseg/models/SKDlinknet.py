...
class SKDblock(nn.Module):
    def __init__(self, channel):
        super(SKDblock, self).__init__()
        # global pooling
    ...
    def forward(self, input):
        ...
        #dilate5_out = self.bn(self.dilate4(nonlinearity(dilate4_out)))
        sum_dilate = dilate1_out + dilate2_out + dilate3_out + dilate4_out #+ dilate5_out
        cat_dilate = torch.cat([dilate1_out.unsqueeze(1), dilate2_out.unsqueeze(1), \
                                dilate3_out.unsqueeze(1),dilate4_out.unsqueeze(1)], 1)

        sum_dilate_gp = nn.AvgPool2d(sum_dilate.shape[2:])(sum_dilate).squeeze_()
        feature_z = nonlinearity(self.fc(sum_dilate_gp))
        ...
        out = (cat_dilate * attention_vectors)
        out = out.sum(dim = 1)
...
class SKDlinkNet(nn.Module):
    def __init__(self, n_classes=2):
        #super(DinkNet34_more_dilate, self).__init__()
        super(SKDlinkNet, self).__init__()
