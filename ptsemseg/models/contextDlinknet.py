...
class ContextPyramidBlock(nn.Module):
    def __init__(self, channel):
        super(ContextPyramidBlock, self).__init__()
        self.conv = nn.Conv2d(channel, channel, 1, 1)
        self.bn = nn.BatchNorm2d(channel)

        # self.channelreduction = nn.Conv2d(channel, channel//4, 1, 1)
    ...
    def forward(self, input):
        ...
        # x = nonlinearity(self.bnreduction(x))
        ...
        # out = torch.cat([dilate1_out, dilate2_out, \
        #                  dilate3_out, dilate4_out], 1)

        out = dilate4_out + dilate3_out + dilate2_out + dilate1_out
        # out = self.conv(out)
        # out = self.bn(out)
        finalout = nonlinearity(x_gp + out*x_master)
...
class ContextNet(nn.Module):
    def __init__(self, n_classes=2):
        ...
        self.dblock = ContextPyramidBlock(256)
