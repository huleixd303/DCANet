...
class SARdata_Loader(data.Dataset):
    ...
    def __init__(
        ...
        self.normlize = transforms.Normalize([0.485, 0.456, 0.406],
                                             [0.229, 0.224, 0.225])
    ...
    def transform(self, img, lbl):
        ...
        if self.img_size == ('same', 'same'):
            ...
            img = img.resize((self.img_size[0], self.img_size[1]), resample=Image.BILINEAR)
            lbl = lbl.resize((self.img_size[0], self.img_size[1]))
        img = self.totensor(img)
        # one channel for Unet
        c, h, w = img.shape
        if c == 1:
    ...
    def encode_segmap(self, mask):
        ...
        label_mask = (mask/255).astype(int)
        return label_mask
    ...
    def setup_annotations(self):
        ...
        # assert expected == 4356, "unexpected dataset sizes"
        assert expected == 420, "unexpected dataset sizes"
