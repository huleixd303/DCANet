...
class MiniSARLoader(data.Dataset):
    ...
    def __init__(self, root, split='train', is_transform=False,
                 img_size=256, augmentations=None, img_norm=True):
    ...
    def encode_segmap(self, mask):
        ...
        mask = mask.astype(int)
        '''
        lable_mask = np.zeros((mask.shape[0], mask.shape[1]), dtype=np.int16)
        lable_mask[np.where(np.all(mask == 255))[:2]] = 1
        lable_mask = lable_mask.astype(int)
        '''
        mask = mask / 255
        mask = mask.astype(int)
        return mask
    ...
    def setup_annotations(self):
        ...
        assert expected == 576, "unexpected dataset sizes"
